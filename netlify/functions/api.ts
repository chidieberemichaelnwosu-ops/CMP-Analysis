...
// Support multiple routing variations to prevent path resolution issues in serverless runtimes
const analyzeRoutes = ["/api/analyze", "/analyze", "/.netlify/functions/api/analyze"];
const healthRoutes = ["/api/health", "/health", "/.netlify/functions/api/health"];

// AI Analysis Endpoint - Proxies requests to Gemini securely in serverless environment
app.post(analyzeRoutes, async (req, res) => {
  try {
    if (!aiClient) {
      // Lazy re-check to pick up runtime environment variables in serverless context
      ...
      if (runtimeKey) {
        ...
        return res.status(500).json({
          error: "Gemini API Client is not configured. Please add your GEMINI_API_KEY in your Netlify Environment Variables."
        });
      }
    }
    ...
    res.status(500).json({ error: error.message || "An error occurred during serverless AI analysis." });
  }
});

// Health check endpoint
app.get(healthRoutes, (req, res) => {
  res.json({ status: "healthy", environment: "netlify-serverless", time: new Date().toISOString() });
});
